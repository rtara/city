...
<script language="javascript" type="text/javascript">
	jQuery(document).ready(function($) {
        ...
        $("#city_about").charCount({
            allowed: {$oConfig->GetValue('module.city.about_len')} ,
            warning: 20
        });
	} );
</script>
...
{/if}
	...
	<label>{$aLang.plugin.city.city_info_staff}</label>
{assign var="form_id" value="1"}
	...
	<a href="#" class="add-staff-form dotted" onclick="return ls.city.addStaff();">{$aLang.plugin.city.city_edit_staff_add}</a><br /><br /><br />
...
{/if}
	<p>
		<span class="form">{$aLang.plugin.city.city_edit_logo}:</span><br /> <input type="file" name="logo" ><br /><br />
	{if $oCity->getLogo()}
	...
	<p>
		<span class="form">{$aLang.plugin.city.city_edit_file}:</span><br /> <input type="file" name="city_file"><br />
	</p>
	<p>
	{if $oCity->getFileName()}
		{$aLang.plugin.city.city_info_file}: <a href="{$oCity->getFilePath()}">{$oCity->getFileName()}</a>
