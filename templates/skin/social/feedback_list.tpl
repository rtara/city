<div class="comments comment-list">
	{foreach from=$aComments item=oComment}
		...
		<div class="comment-path">
			<a href="{$oCity->getUrlFull()}">{$oCity->getName()|escape:'html'}</a> /
			<a href="{$oCity->getUrlFull()}/feedbacks/" class="blog-name">{$aLang.block_stream_feedbacks}</a>
			<a href="{$oCity->getUrlFull()}/feedbacks/" class="comments-total">{$oCity->getCountFeedback()}</a>
		</div>
		...
		<section class="comment">
			...
			<ul class="comment-info clearfix">
				...
				<li class="comment-date">
					<a href="{if $oConfig->GetValue('module.comment.nested_per_page')}{router page='comments'}{else}{$oCity->getUrlFull()}#comment{/if}{$oComment->getId()}" class="link-dotted" title="{$aLang.comment_url_notice}">
